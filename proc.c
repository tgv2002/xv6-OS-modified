...
#ifdef MLFQ       // Relevant only if MLFQ
	p->cur_q = 0;     
#endif
...
exit(void)
{
	...
	// Process ends here, hence end time is updated
	curproc->etime = ticks;

	if(POLICYCOMP && curproc->pid == 3)
	{
		cprintf("The execution in this scheduling policy consumed a total of %d timer ticks\n",
...
allTimeUpdate()
{
	/* Increments the waiting time of a process by one time unit (for one tick) */
...
mlfqUpdates()
{
	/* updates the ticks for processes in multi-level feedback queue */
	...
	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
	{
		if(p->state != RUNNING)       // no update is needed if process is not running
			continue;
		p->curr_ticks++;              // one tick passed in current time slice
		if(p->cur_q >= 0 && p->cur_q <= 4)
			p->num_ticks[p->cur_q]++;     // Current queue's ticks are updated
	}
	release(&ptable.lock);
#endif
}
...
ageHere()
{
	/* implements ageing, i.e, checks if waiting time exceeds a threshold. If yes, its level is promoted in the queue */
...
// Wait for a child process to exit and return its pid.
// Return -1 if this process has no children.
// Additionally, return the waiting time and run time of
// the corresponding child process
	int
waitx(int* wtime, int* rtime)
{
	struct proc *p;
	int havekids, pid, total_time;
	struct proc *curproc = myproc();
...
scheduler(void)
{
	...
	for(;;){
		...
		if(highPriority != 0)
		{
			for(p = ptable.proc; p < &ptable.proc[NPROC] && foundBetter == 0; p++)
			{
				if(p->state != RUNNABLE)     
					continue;

				if(p->priority == highPrior)
				{
					// Switch to chosen process.  It is the process's job
					// to release ptable.lock and then reacquire it
					// before jumping back to us.
		...
		for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
			...
			if(p->curr_ticks >= slice_size[p->cur_q])
			{
				if(p->cur_q == 4)
					continue;
				p->cur_q++;
		...
		ageHere();
		int processFound = 0;
		struct proc* runThis = 0;

		/*if(prevExecProc != 0 && prevExecProc->state == RUNNABLE && prevExecProc->curr_ticks < slice_size[prevExecProc->cur_q])
		  {
		  processFound = 1;
		  runThis = prevExecProc;		// non-preemptive
		  }*/
